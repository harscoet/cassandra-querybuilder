...
var QueryBuilder = function (queryType, keyspace, client) {
...
QueryBuilder.prototype.getQuery = function () {
  ...
  if (_this.queryType === 1) {
    ...
    query  = 'INSERT INTO ' + _this.keyspace +' (' + _this.fields.join(',') + ') ';
    query += 'VALUES (' + params + ');';
  } else {
    if (_this.queryType === 2) {
      query = 'UPDATE ' + _this.keyspace + ' SET ';
      ...
      query = 'SELECT ' + fields + ' FROM ' + _this.keyspace;
    }
...
QueryBuilder.prototype.execute = function (options, next) {
  ...
  this.client.execute(this.getQuery(), this.getParams(), options, function (err) {
    if (err) err = {
      message: err.message,
